// Load .env.local first (local dev), then .env for any vars not already set.
// override:false means the first file to set a var wins — local always beats prod.
...
app.get('/api/tickets/export/xlsx', requireAuth, async (req, res) => {
  ...
  const ws = wb.addWorksheet('Time Tickets');
  ws.columns = [{width:12},{width:14},{width:24},{width:12},{width:16},{width:20},{width:10},{width:10},{width:10},{width:10},{width:36},{width:20},{width:20},{width:12}];
  xlsxRows.forEach(r => ws.addRow(r));
...
app.get('/api/tickets/:id/export/xlsx', requireAuth, async (req, res) => {
  ...
  const wb=new ExcelJS.Workbook(); const ws=wb.addWorksheet('Time Ticket');
  ws.columns=[{width:18},{width:22},{width:14},{width:10},{width:10}];
  xlsxRows.forEach(r=>ws.addRow(r));
...
app.get('/api/po/export/xlsx', requireAdmin, async (req, res) => {
  ...
  const wb=new ExcelJS.Workbook(); const ws=wb.addWorksheet('Purchase Orders');
  ws.columns=[{width:18},{width:14},{width:18},{width:16},{width:24},{width:18},{width:36},{width:16},{width:12},{width:12},{width:30},{width:22}];
  xlsxRows.forEach(r=>ws.addRow(r));
...
app.get('/api/project-folders/:id/export/xlsx', requireAdmin, async (req, res) => {
  ...
  const tws=wb.addWorksheet('Time Tickets'); tws.columns=[{width:12},{width:18},{width:16},{width:20},{width:20},{width:10},{width:10},{width:10},{width:36},{width:20},{width:20},{width:12}];
  tRows.forEach(r=>tws.addRow(r));
  // POs sheet
  ...
  const pws=wb.addWorksheet('Purchase Orders'); pRows.forEach(r=>pws.addRow(r));
...
app.get('/api/timesheets/export/all', requireAdmin, async (req, res) => {
  ...
  for (const u of users) {
    ...
    const ws=wb.addWorksheet(ts.user.name.slice(0,31)); ws.columns=[{width:12},{width:6},{width:14},{width:10},{width:12},{width:22},{width:12},{width:24}];
...
app.get('/api/timesheets/:userId/export', requireAdmin, async (req, res) => {
  ...
  const ws = wb.addWorksheet(ts.user.name.slice(0,31));
  ws.columns = [{width:12},{width:6},{width:14},{width:10},{width:12},{width:22}];
  rows.forEach(r => ws.addRow(r));
  const buf = await wb.xlsx.writeBuffer();
  res.setHeader('Content-Type','application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
...
(async () => {
  try {
    // Print which database we're pointed at so it's always visible at startup
    const dbUrl  = process.env.DATABASE_URL || '';
    const dbHost = (() => { try { return new URL(dbUrl).hostname; } catch { return dbUrl; } })();
    const isLocal = ['localhost', '127.0.0.1', '::1'].includes(dbHost);

    // HTTPS support for local dev + phone testing (certs generated by npm run https:setup)
    const fs   = require('fs');
    ...
    if (isLocal && hasCerts) {
      ...
      https.createServer(creds, app).listen(HTTPS_PORT, '0.0.0.0', () => {
        const lanIp = Object.values(os.networkInterfaces()).flat()
          .find(i => i.family === 'IPv4' && !i.internal)?.address || 'your-lan-ip';
        console.log(`\n  J&D Western Electric — Field Operations Hub`);
        console.log(`  ⚠  DATABASE: ${dbHost} (LOCAL)`);
        console.log(`  HTTP  → http://localhost:${PORT}`);
        console.log(`  HTTPS → https://localhost:${HTTPS_PORT}`);
        console.log(`  Phone → https://${lanIp}:${HTTPS_PORT}\n`);
      });
    } else {
      app.listen(PORT, '0.0.0.0', () => {
        const tag = isLocal ? '(LOCAL)' : '(PRODUCTION ⚠)';
        console.log(`\n  J&D Western Electric — Field Operations Hub`);
        console.log(`  DATABASE: ${dbHost} ${tag}`);
